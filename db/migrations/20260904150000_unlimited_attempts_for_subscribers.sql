...
DECLARE v_uid uuid := auth.uid(); e public.exams; v_id uuid; v_used int; v_today int;
        ...
        v_has_sub boolean := public.has_active_subscription(v_uid);
BEGIN
  ...
  -- daily limit: one attempt per exam per day (Tehran calendar day), waived for admins and subscribers
  IF NOT public.is_admin() AND NOT v_has_sub THEN
  ...
  -- max-attempts limit, waived for admins and subscribers
  SELECT count(*) INTO v_used FROM public.exam_attempts
   WHERE exam_id = p_exam_id AND candidate_id = v_uid AND status <> 'in_progress';
  IF e.max_attempts IS NOT NULL AND v_used >= e.max_attempts AND NOT public.is_admin() AND NOT v_has_sub THEN
    RAISE EXCEPTION 'max attempts reached';
  END IF;

  IF NOT e.is_free AND NOT v_has_sub AND NOT public.is_admin() THEN
